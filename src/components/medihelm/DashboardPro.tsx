...
export function DashboardPro() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setMedicaments(Array.isArray(meds) ? meds.slice(0, 8).map((m: Record<string, unknown>) => ({
        id: m.id as string,
        dci: (m.dci as string) || '',
        nomCommercial: (m.nomCommercial as string) || '',
        prixPublic: (m.prixPublic as number) || 0,
        stockMinimum: (m.stockMinimum as number) || 5,
        lots: Array.isArray(m.lots) ? m.lots as { quantite: number; dateExpiration: string }[] : [],
      })) : [])
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {medicaments.map((med) => {
                          ...
                          const isLow = stock <= (med.stockMinimum || 5)
                          return (
                            <tr key={med.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                              ...
                              <td className="py-3 px-2 text-right font-medium">{(med.prixPublic || 0).toLocaleString('fr-FR')} FCFA</td>
